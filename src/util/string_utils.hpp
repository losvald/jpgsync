...
inline bool StartsWith(const std::string& subject, const std::string& prefix) {
  return prefix.length() <= subject.length() &&
      subject.compare(0, prefix.length(), prefix) == 0;
}

inline bool EndsWith(const std::string& subject, const std::string& suffix) {
  return suffix.length() <= subject.length() &&
      subject.compare(subject.length() - suffix.length(), suffix.length(),
                      suffix) == 0;
}

bool EndsWithin(std::string subject, size_t read_count,
